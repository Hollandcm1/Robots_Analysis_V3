...
build_data_long_exp2 <- function(data, codes) {
  ...
  for (p_num in participants) {
    ...
    for (trial_num in trials) {
      ...
      print(paste("---Trial", trial_num))
      
      trial_data <- data[[p_num]][[trial_num]]
      
      force <- as.data.frame(trial_data['force_input'])
      
      if (nrow(force) == 0) {
        warning(paste("Participant", p_num, "Trial", trial_num, "has no force data"))
        next
      }
