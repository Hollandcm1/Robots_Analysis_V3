# plot_all_trial_force_by_proximity
...
plot_all_trial_force_by_proximity <- function (data, maps) {
  ...
  data$force_proximity <- data$force_magnitude / data$closest_object_distance
  ...
  for (p_num in unique(data$participant)) {
    ...
    for (t_num in unique(participant_data$trial)) {
      ...
      # force by proximity layer
      #trial_data$force_proximity <- trial_data$force_magnitude / trial_data$closest_object_distance
      g1 <- geom_point(data = trial_data, aes(x = position_x_robot, y = position_y_robot, colour = force_proximity))
      ...
      ggsave(here('output', 'figures_force_by_proximity', paste("participant", p_num), paste("_trial", t_num, ".png")), plot = final_plot, device = "png")
